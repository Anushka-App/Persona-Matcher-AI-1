...
// Save results to file for easier viewing
fs.writeFileSync('recommendations-output.json', JSON.stringify(result, null, 2));

console.log('Results saved to recommendations-output.json');
console.log('\nSample of recommendations:');
console.log(`Suggested Bag Type: ${result.suggestedBagType}`);
console.log(`Confidence: ${result.confidence}%`);
console.log(`Total Products: ${result.summary.totalProducts}`);
console.log(`Average Match Score: ${result.summary.averageMatchScore}%`);
console.log(`Confidence Level: ${result.summary.confidenceLevel}`);

console.log('\nTop 3 recommendations:');
result.recommendations.slice(0, 3).forEach((rec, index) => {
    console.log(`${index + 1}. ${rec.productName} (${rec.productType}) - Score: ${rec.matchScore}`);
});

console.log('\n' + '='.repeat(80) + '\n');
console.log('System successfully processed user inputs and generated recommendations!');
console.log('Full results available in recommendations-output.json');
