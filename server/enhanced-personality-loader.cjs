...
class EnhancedPersonalityLoader {
    ...
    async loadPersonalityDataset() {
        return new Promise((resolve, reject) => {
            const csvPath = path_1.default.join(process.cwd(), 'updated_ml_bags_personality_dataset_cleaned.csv');
    ...
    async loadArtworkDataset() {
        return new Promise((resolve, reject) => {
            const excelPath = path_1.default.join(process.cwd(), '..', 'mapped_persona_artwork_data.xlsx');
    ...
    processEnhancedPersonalityTypes() {
        const personalityGroups = new Map();
    ...
    findMatchingArtwork(personalityName) {
        ...
        return matchingArtwork.slice(0, 12);
    }
    isPersonalityMatch(personalityName, traitsText) {
    ...
    extractPersonalityDescription(traitsText, productName) {
    ...
    extractUniqueQuestions(entries) {
    ...
    findMatchingBags(answers, personalityType) {
        const matchedBags = [];
    ...
    generateDetailedAnalysis(personalityType, selectedOptions, answers) {
    ...
    getPersonalityTraits(personalityName) {
        const traitMap = {
    ...
    generateStrengths(personalityName) {
        const strengthMap = {
            'Savannah Spirit': [
    ...
    generateInsights(personalityName) {
        const insightMap = {
            'Savannah Spirit': [
    ...
    generateRecommendations(personalityName) {
    ...
    generateStyleRecommendations(personalityName) {
        const styleMap = {
            'Savannah Spirit': [
    ...
    generateLifestyleAdvice(personalityName) {
        const lifestyleMap = {
            'Savannah Spirit': [
    ...
    generateProducts(personalityName) {
        const productMap = {
            'Savannah Spirit': [
    ...
    generateSentiment(personalityName) {
        const sentimentMap = {
...
exports.default = enhancedPersonalityLoader;
