...
exports.PersonalityReportService = void 0;
class PersonalityReportService {
    constructor(llmService) {
        this.llmService = llmService;
    }
    async generatePersonalityReport(userInput) {
        try {
            ...
            return this.parseTextResponse(response.data, userInput);
        }
        catch (error) {
    ...
    createPersonalityReportPrompt(userInput) {
        return `
...
`;
    ...
    parseTextResponse(text, userInput) {
    ...
    generateFallbackReport(userInput) {
        return {
    ...
    analyzeCommunicationStyle(style) {
        const styleLower = style.toLowerCase();
        if (styleLower.includes('direct'))
            return 'expresses thoughts clearly and honestly';
        if (styleLower.includes('empathetic'))
            return 'shows understanding and emotional intelligence';
        if (styleLower.includes('analytical'))
            return 'approaches conversations with logic and structure';
        if (styleLower.includes('collaborative'))
            return 'seeks input and builds consensus';
        return 'communicates in their own unique way';
    }
    getCommunicationImpact(style) {
        const styleLower = style.toLowerCase();
        if (styleLower.includes('direct'))
            return 'create clear expectations and avoid misunderstandings';
        if (styleLower.includes('empathetic'))
            return 'build strong emotional connections';
        if (styleLower.includes('analytical'))
            return 'solve complex problems effectively';
        if (styleLower.includes('collaborative'))
            return 'foster teamwork and cooperation';
        return 'create positive interactions in various contexts';
    }
    analyzeMediaPreferences(media) {
        if (media.some(m => m.toLowerCase().includes('fiction')))
            return 'imagination and storytelling';
        if (media.some(m => m.toLowerCase().includes('non-fiction')))
            return 'learning and knowledge';
        if (media.some(m => m.toLowerCase().includes('drama')))
            return 'emotional depth and human experience';
        if (media.some(m => m.toLowerCase().includes('comedy')))
            return 'humor and lightheartedness';
        return 'diverse perspectives and experiences';
    }
    getMediaInsights(media) {
        if (media.some(m => m.toLowerCase().includes('classic')))
            return 'appreciation for timeless themes';
        if (media.some(m => m.toLowerCase().includes('modern')))
            return 'openness to contemporary ideas';
        if (media.some(m => m.toLowerCase().includes('international')))
            return 'cultural curiosity and global perspective';
        return 'broad interests and open-mindedness';
    }
    analyzeStrengths(strengths) {
        if (strengths.some(s => s.toLowerCase().includes('leadership')))
            return 'natural leadership abilities';
        if (strengths.some(s => s.toLowerCase().includes('creativity')))
            return 'innovative thinking and problem-solving';
        if (strengths.some(s => s.toLowerCase().includes('communication')))
            return 'strong interpersonal skills';
        if (strengths.some(s => s.toLowerCase().includes('organization')))
            return 'excellent planning and execution abilities';
        return 'valuable personal qualities';
    }
    getGrowthStrategies(weaknesses) {
        if (weaknesses.some(w => w.toLowerCase().includes('patience')))
            return 'mindfulness and stress management techniques';
        if (weaknesses.some(w => w.toLowerCase().includes('confidence')))
            return 'positive self-talk and skill development';
        if (weaknesses.some(w => w.toLowerCase().includes('organization')))
            return 'time management tools and systems';
        if (weaknesses.some(w => w.toLowerCase().includes('communication')))
            return 'active listening and feedback practice';
        return 'targeted skill development and self-reflection';
    }
}
exports.PersonalityReportService = PersonalityReportService;
