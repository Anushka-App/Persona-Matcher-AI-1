...
export class PersonalityGeminiService {
  ...
  constructor(dataService: PersonalityDataService) {
    ...
    try {
      ...
      // Use the correct model name - gemini-1.5-flash is the current recommended model
  ...
  async analyzePersonalityAndRecommend(
    userAnswers: Array<{ question: string; selected_personality: string }>,
    availableBags: BagRecommendation[],
    ...
    const prompt = this.createAdvancedFashionPrompt(
      userAnswers,
      availableBags,
      personalityTypes,
      artworkData
  ...
  private createAdvancedFashionPrompt(
    userAnswers: Array<{ question: string; selected_personality: string }>,
    availableBags: BagRecommendation[],
    personalityTypes: string[],
    artworkData: ArtworkData[]
...
`;
  ...
  private parseGeminiResponse(response: string, availableBags: BagRecommendation[]): {
    personality_analysis: PersonalityAnalysis;
    recommendations: EnhancedRecommendation[];
  } {
    try {
      ...
      // Validate and enhance recommendations with actual bag data
      const enhancedRecommendations = parsed.recommendations.map((rec: Record<string, unknown>) => {
        const actualBag = availableBags.find(bag =>
          bag.Bag_Name.toLowerCase() === String(rec.bag_name || '').toLowerCase()
        );
  ...
  private generateFallbackResult(
    ...
    const predictedPersonality = Object.keys(personalityCounts).reduce((a, b) =>
      personalityCounts[a] > personalityCounts[b] ? a : b
    );

    // Get bags that match the personality type
    let matchingBags = availableBags.filter(bag =>
      bag.Personality_Description.toLowerCase().includes(predictedPersonality.toLowerCase())
    );

    // If no matching bags found, get some general recommendations
    ...
    // Get all products from the same artwork collection
    // Find the most common artwork/collection and get all products from it
    const artworkGroups = new Map<string, BagRecommendation[]>();

    matchingBags.forEach(bag => {
      // Extract artwork/collection from bag name or description
    ...
    // Get the largest artwork group
    let largestGroup: BagRecommendation[] = [];
    ...
    // Limit to 8 unique products for better variety
  ...
  private generateDetailedPersonalityAnalysis(
    personality: string,
    userAnswers: Array<{ question: string; selected_personality: string }>
  ): {
    ...
    const insight = personalityInsights[personality as keyof typeof personalityInsights] || fallback;
  ...
  private getStylePreferences(personality: string): string[] {
    const styleMap = {
    ...
    return styleMap[personality as keyof typeof styleMap] || ['Versatile', 'Classic', 'Practical', 'Elegant', 'Unique'];
  }

  private getLifestyleInsights(personality: string): string {
    const lifestyleMap = {
    ...
    return lifestyleMap[personality as keyof typeof lifestyleMap] || 'You have a unique approach to life that reflects your distinctive personality and values.';
  }

  private generatePersonalizedReasoning(bag: BagRecommendation, personality: string): string {
    // Use artwork personality data instead of generic descriptions
    // This will be overridden by the main recommendations endpoint
    return `This ${bag.Bag_Name} reflects your unique personality and style preferences.`;
  }

  private getStyleMatch(bag: BagRecommendation, personality: string): string {
    const styleMatchMap = {
    ...
    return styleMatchMap[personality as keyof typeof styleMatchMap] ||
      'Reflects your unique style preferences and personality';
  }

  private getOccasionSuitability(personality: string): string {
    const occasionMap = {
    ...
    return occasionMap[personality as keyof typeof occasionMap] ||
