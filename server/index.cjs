...
const data_loader_1 = require("./data-loader.cjs");
const recommender_1 = require("./recommender.cjs");
const artwork_persona_mapping_1 = require("./artwork-persona-mapping.cjs");
const artwork_personality_descriptions_1 = require("./artwork-personality-descriptions.cjs");
const personality_quiz_service_1 = require("./personality-quiz-service.cjs");
const PersonalityAnalysisService_1 = require("./services/PersonalityAnalysisService.cjs");
const DatabaseService_1 = require("./services/DatabaseService.cjs");
const ArtworkPersonalityService_1 = require("./services/ArtworkPersonalityService.cjs");
const PersonalityReportCSVService_1 = require("./services/PersonalityReportCSVService.cjs");
const UserService_1 = __importDefault(require("./services/UserService.cjs"));
const LLMService_1 = require("./services/LLMService.cjs");
const FashionRecommendationService_1 = require("./services/FashionRecommendationService.cjs");
...
async function startServer() {
    try {
        ...
        const fashionRecommendationService = new FashionRecommendationService_1.FashionRecommendationService(llmService, productDatabase);
        ...
        const getFallbackAlternatives = (artworkTheme) => {
            const themeMap = {
...
Write in a warm, personal tone as if speaking directly to the person.`;
            ...
            try {
                ...
                if (artworkTheme && bagPref) {
                    ...
                    matches = productDatabase.filter(p => {
                        ...
                        const excelThemes = p.personalityTraits?.toLowerCase() || '';
                        const artworkLower = p.artworkName?.toLowerCase() || '';
                ...
                if (matches.length === 0 && artworkTheme) {
                    ...
                    matches = productDatabase.filter(p => {
                        ...
                        if (themeLower === 'animal') {
                            ...
                            return animalKeywords.some(keyword => traitsLower.includes(keyword) || artworkLower.includes(keyword));
                        }
                ...
                if (matches.length > 12) {
                    const uniqueMatches = matches.filter((product, index, self) => index === self.findIndex(p => p.artworkName === product.artworkName &&
                    ...
                    const remainingProducts = productDatabase
                        .filter(p => !uniqueMatches.some(m => m.artworkName === p.artworkName && m.productName === p.productName))
        ...
        app.post('/api/personality/personality-only-report', async (req, res) => {
            try {
                ...
                let markdown_report = '';
                let llmUsed = false;
                const includeGrowthTips = false;
                const includeVisualSummary = false;
...
`;
            ...
            try {
                ...
                switch (type) {
                    case 'personality-only':
                        const quizAnswers = data.quizJourney?.map((j) => `${j.question}: ${j.selectedOption}`).join(', ') ||
        ...
        app.post('/api/fashion-recommendations', async (req, res) => {
            try {
        ...
        app.post('/api/artwork-analysis', async (req, res) => {
            try {
                const { artworkTheme, productType, artworkPreferences, personality, sentiment } = req.body;
...
No additional text outside JSON.`;
                ...
                res.status(200).json({
                    success: true,
                    suggestedBagType: req.body.productType || 'crossbody',
                    confidence: 85,
        ...
        app.post('/api/personality-report/generate', async (req, res) => {
            try {
                const { PersonalityReportController } = await Promise.resolve().then(() => __importStar(require('./controllers/personalityReportController.cjs')));
            ...
            for (const row of rawData) {
                try {
                    ...
                    const artwork = {
                        ...
                        primaryTheme: cleanString(row['Artwork Name']),
                        ...
                        designElements: '',
                        overallPersonality: 'Distinctive and creative',
                        buyerPersonalityMatch: 'Creative and artistic individuals',
                        psychologicalAppeal: 'Appeals to those who appreciate unique design'
                    };
